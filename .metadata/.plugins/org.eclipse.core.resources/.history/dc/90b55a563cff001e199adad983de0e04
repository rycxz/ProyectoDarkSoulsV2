...
public class CargaDatosJefes {
	...
	public static ArrayList<Jefes> cargarContenidoJefes() {
		...
		//creo los objetos de los jefes y los introduzco al array list y esto es lo que devuelvo
		ArrayList <Jefes> jefesTotales= new ArrayList<>();


		for (int i = 0; i < numeroJefes; i++) {
			Jefes jefes = new Jefes(nombre[i], tipo[i], informacionBreve[i], vida[i], ataque[i]);
			jefesTotales.add(jefes);
